...
moveRouter.post("/:move", async (req: Request, res: Response) => {
	...
	try {
		...
		const gameOne = (await collections.games?.findOne(
			queryOne
		)) as unknown as Game;

		if (
			(gameOne.blackPlayer === userId && gameOne.fen.split(" ")[1] !== "b") ||
		...
		let fen;
		if (
			gameOne.blackPlayer === "stockfish" ||
			gameOne.whitePlayer === "stockfish"
		) {
			fen = await (
				await stockfish.autoBot(gameOne.fen.split(" "), move)
			).join(" ");
		} else {
			fen = await (
				await stockfish.makeMove(gameOne.fen.split(" "), move)
			).join(" ");
		}
