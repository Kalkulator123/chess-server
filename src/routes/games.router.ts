...
gamesRouter.post("/create/:team/:bot", async (req: Request, res: Response) => {
	...
	try {
		...
		result ? res.status(201).send(response) : res.status(500).send(response);
	} catch (error: any) {
		console.error(error);
	...
	try {
		...
		if (game) {
			...
			const newGame: IGame = {
				...
				status: game.status,
				whitePlayer:
					game.whitePlayer === "" ? req.body.playerId : game.whitePlayer,
				blackPlayer:
					game.blackPlayer === "" ? req.body.playerId : game.blackPlayer,
			};
